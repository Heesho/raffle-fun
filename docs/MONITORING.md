...
4. the recorded prize, sponsor, treasury, reserve, and deadlines match the event;
...
| P1       | `Drawing` reaches callback deadline without a result or refund transaction                                                                                                                     |              15 minutes | submit `enableRefunds` and alert affected users                                                  |
| P2       | active sold raffle remains without `DrawRequested` beyond the agreed keeper SLO                                                                                                                |                  1 hour | notify sponsor/buyers and submit a public draw request                                           |
...
| P3       | stale unclaimed quote/refund liability or sponsor prize; Safe signer/threshold review due                                                                                                      |          1 business day | notify the entitled account or operations owner                                                  |
...
- raffle counts by lifecycle, sales volume, native request fees, fulfillment latency,
  ...
  reconciled to finalized raw logs), and deadline queues;
- unsettled pot, refund liability, sponsor proceeds, protocol fees, actual USDC
  balance, and deficit per raffle and in aggregate;
- winning settlements, refunds, fixed-recipient releases, and sponsor-prize withdrawals.
