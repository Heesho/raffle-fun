...
Copy `apps/web/.env.example`. Ethereum mainnet is the default; Sepolia, Base, Base Sepolia and local Anvil are also supported. Every supplied value is
...
- the advertised ticket price is the total paid; the 5% fee is carved out of it
...
`ThresholdBar` scales to `max(sold, minimum)` and marks the flip point — the ticket
count where the prize switches from the cash pot to the NFT — rather than pinning
the threshold to the end of the track.
