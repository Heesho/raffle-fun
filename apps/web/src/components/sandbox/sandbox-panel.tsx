...
function BuyPanel({
  ...
  const protocolFee = (gross * 5n) / 100n;
  ...
  return (
    <section className="card p-6">
      ...
      <dl className="mt-5 space-y-2 text-sm">
        ...
        <Row label="Protocol · 5%" value={amount(protocolFee)} />
        <Row label="Added to net pot" value={amount(gross - protocolFee)} />
        <Row
          label={
          ...
          {raffle.winningTicketId !== null ? (
            ...
            >
              ...
              <p className="mt-1.5 font-medium text-[var(--ink)]/70">
                {thresholdMet(raffle)
                  ? "The threshold was met, so the winning ticket takes the NFT and the sponsor takes the pot."
                  : "The threshold was missed, so the winning ticket takes 80% of the pot and the NFT goes back to the sponsor."}
              </p>
            </div>
          ) : null}
