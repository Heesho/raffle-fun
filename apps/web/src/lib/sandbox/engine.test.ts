...
describe("sandbox purchasing", () => {
  it("charges exactly the advertised price and carves the fee out of it", () => {
    ...
    // 5% protocol fee comes out of the gross, never added on top.
    expect(updated.netPot).toBe(gross - (gross * 5n) / 100n);
  });
...
describe("sandbox settlement", () => {
  ...
  it("awards the NFT to the winning ticket holder when the threshold is met", () => {
    ...
    // Threshold met: the sponsor takes the entire net pot.
